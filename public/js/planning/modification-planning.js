        ...
        oldEventSet._def.resourceIds.forEach((oldResource) => {
          newEventAppointment._def.resourceIds.forEach((newResource) => {
            if (newResource == oldResource) {
              if (
                newEventAppointment._def.extendedProps.appointment !=
                oldEventSet._def.extendedProps.appointment
              ) {
                if (
                  newEventAppointment._def.publicId == oldEvent._def.publicId
                ) {
                  var currentModifyEvent = calendar.getEventById(
                    oldEvent._def.publicId
                  );
                  if (
                    !(
                      currentModifyEvent.start > oldEventSet.end ||
                      currentModifyEvent.end < oldEventSet.start
                    ) ||
                    (currentModifyEvent.start < oldEventSet.start &&
                      currentModifyEvent.end > oldEventSet.end)
                  ) {
                    isEditable = false;
                  }
                } else {
                  if (
                    !(
                      newEventAppointment.start > oldEventSet.end ||
                      newEventAppointment.end < oldEventSet.start
                    ) ||
                    (newEventAppointment.start < oldEventSet.start &&
                      newEventAppointment.end > oldEventSet.end)
                  ) {
                    isEditable = false;
                  }
                }
        ...
        if (newEventAppointment._def.publicId == oldEventSet._def.publicId) {
          if (newEventAppointment._def.publicId == oldEvent._def.publicId) {
            calendar.getEventById(
              oldEvent._def.publicId
            )._def.ui.backgroundColor = RessourcesAllocated(
              calendar.getEventById(oldEvent._def.publicId)
            );
            calendar.getEventById(oldEvent._def.publicId)._def.ui.borderColor =
              RessourcesAllocated(
                calendar.getEventById(oldEvent._def.publicId)
            ...
            var startDate = new Date(
              oldEvent.start.getTime() - 2 * 60 * 60 * 1000
            );
            var startStr = formatDate(startDate).replace(" ", "T");
            ...
            newEventAppointment._def.ui.backgroundColor =
              RessourcesAllocated(oldEventSet);
            newEventAppointment._def.ui.borderColor =
              RessourcesAllocated(oldEventSet);
            var startDate = new Date(
              oldEventSet.start.getTime() - 2 * 60 * 60 * 1000
            );
            var startStr = formatDate(startDate).replace(" ", "T");
            var endDate = new Date(
              oldEventSet.end.getTime() - 2 * 60 * 60 * 1000
            );
            var endStr = formatDate(endDate).replace(" ", "T");
